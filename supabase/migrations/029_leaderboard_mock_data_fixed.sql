-- 029_leaderboard_mock_data_fixed.sql
-- Comprehensive mock data for leaderboard testing
-- FIXED: Works with Supabase Auth schema (no password_hash column)

BEGIN;

-- ===================================
-- PART 0: ENSURE SCHEMA COMPATIBILITY
-- ===================================

-- Add password_hash column if it doesn't exist (for standalone mode)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Use INSERT with ON CONFLICT to handle existing users
...
-- Assign first_trade badge to all users who don't have it yet
INSERT INTO public.user_badges (user_id, badge_id, awarded_at)
SELECT u.id, 'first_trade', u.created_at + INTERVAL '1 day'
FROM public.users u
WHERE u.id NOT IN (SELECT user_id FROM public.user_badges)
  AND (u.id LIKE 'a%' OR u.id LIKE 'b%' OR u.id LIKE 'c%' OR u.id LIKE 'd%' OR u.id LIKE 'e%')
ON CONFLICT (user_id, badge_id) DO NOTHING;
