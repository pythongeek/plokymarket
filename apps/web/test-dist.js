...
async function stressTestWallet(userId) {
  const sb = createClient(SUPABASE_URL, SUPABASE_KEY);
  console.log(`Starting stress test for user: ${userId}`);
  const { data: before } = await sb.from("wallets").select("balance, locked_balance").eq("user_id", userId).single();
  console.log("Before balance:", before?.balance);
  console.log("Before locked:", before?.locked_balance);
  const dummyMarketId = "77cecbe8-3683-4743-998a-9c700dc72580";
  const requests = Array(10).fill(null).map(
    (_, i) => sb.rpc("place_order_atomic", {
      p_user_id: userId,
...
Results: ${successCount} successful, ${failureCount} failed (insufficient balance or locks)`);
  ...
  const { data: after } = await sb.from("wallets").select("balance, locked_balance").eq("user_id", userId).single();
  ...
  const intact = Math.abs(initialTotal - finalTotal) < 1e-4;
  console.log(intact ? "\u2705 Financial integrity perfectly maintained." : `\u274C INTEGRITY VIOLATION DETECTED! Expected ${initialTotal}, Got ${finalTotal}`);
}
