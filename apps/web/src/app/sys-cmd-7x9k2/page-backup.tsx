...
function RiskManagementCard() {
  const { riskControls, fetchRiskControls, updateRiskControls } = useStore();
  const [isUpdating, setIsUpdating] = useState(false);
  const { t } = useTranslation();
  ...
  const handleToggleHalts = async (type: 'trading' | 'withdrawals', enabled: boolean) => {
    setIsUpdating(true);
    await updateRiskControls({
      [`${type}_halted`]: enabled
    });
    setIsUpdating(false);
  };
  ...
  return (
    <Card className="border-red-200 bg-red-50/10 dark:bg-red-900/5">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label className="text-sm font-medium">Halt All Trading</Label>
            <p className="text-[10px] text-muted-foreground">Stop all new orders platform-wide</p>
          </div>
          <Switch
            checked={riskControls.trading_halted}
            onCheckedChange={(checked) => handleToggleHalts('trading', checked)}
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label className="text-sm font-medium">Halt Withdrawals</Label>
            <p className="text-[10px] text-muted-foreground">Freeze all fund movements</p>
          </div>
          <Switch
            checked={riskControls.withdrawals_halted}
            onCheckedChange={(checked) => handleToggleHalts('withdrawals', checked)}
            className="data-[state=checked]:bg-red-600"
            disabled={isUpdating}
          />
        </div>
        {riskControls.trading_halted && (
          <div className="p-2 bg-red-100 dark:bg-red-900/20 rounded text-[10px] text-red-600 font-bold uppercase tracking-wider text-center">
            Trading Halt is ACTIVE
          </div>
        )}
      </CardContent>
    </Card>
...
export default function AdminPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Risk & Maintenance Controls (Top Row) */}
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <Badge variant="outline" className="text-[10px] text-green-500 border-green-500">Perfect</Badge>
            </div>
          </CardContent>
        </Card>
      </div>
