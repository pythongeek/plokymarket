...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!manualOverride && new Date(event.trading_closes_at || 0) > new Date()) {
        ...
        if (event.status === 'resolved' || (event.markets as any)[0].status === 'resolved') {
        ...
        try {
            result = await executeVerificationWorkflow(eventId, wId, {
                ...
                tradingEnds: event.trading_closes_at ? new Date(event.trading_closes_at) : new Date()
            });
        } catch (execError: any) {
        ...
        if ((result.outcome === 'yes' || result.outcome === 'no') && result.confidence >= 80) {
            ...
            await supabase.from('events').update({ status: 'paused' as any }).eq('id', eventId);
            await supabase.from('markets').update({ status: 'paused' as any }).eq('id', marketId);
