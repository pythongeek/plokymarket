...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    executionId = await createWorkflowExecution('exchange_rate_update' as any, { timestamp: data.timestamp });
    if (executionId) {
    ...
    const { data: rawConfig, error: configError } = await supabase
      .from('exchange_rate_config' as any)
      .select('*')
      .eq('id', 1)
      .single();
    const config = rawConfig as unknown as ExchangeRateConfig;
    ...
    if (rate < config.min_usdt_to_bdt || rate > config.max_usdt_to_bdt) {
      ...
      await supabase.from('exchange_rate_history' as any).insert({
        usdt_to_bdt: rate,
        bdt_to_usdt: 1 / rate,
