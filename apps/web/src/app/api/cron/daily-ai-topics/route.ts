...
async function verifyQStashSignature(request: NextRequest): Promise<boolean> {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[Cron] Development mode - allowing request');
    return true;
  }
...
export async function POST(request: NextRequest) {
  ...
  console.log('[Cron] Environment check:', {
    ...
    MASTER_CRON_SECRET_SET: !!process.env.MASTER_CRON_SECRET,
...
async function processDailyAiTopics(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (settingsError) {
      console.error('[DailyTopics] Failed to fetch AI settings:', settingsError);
      throw new Error(`Failed to fetch AI settings: ${settingsError.message}`);
    }

    if (!settings) {
      console.error('[DailyTopics] No settings found in admin_ai_settings table');
      throw new Error('No AI settings found - table may be empty');
    }

    // Check if auto-generate is enabled
    if (!settings.auto_generate_enabled) {
      console.log('[DailyTopics] Auto-generation is disabled in settings');
      return NextResponse.json({
        success: true,
...
Generate exactly ${count} topics.`;
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.error('[DailyTopics] GEMINI_API_KEY not configured!');
