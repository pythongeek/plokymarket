...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        for (const market of markets) {
            ...
            const now = new Date();
            const pending = market.pending_tick_change as any;

            if (pending && new Date(pending.apply_at) <= now) {
                // Apply the change
                await supabase.from('markets').update({
                    current_tick: Number(pending.new_tick),
            ...
            } else if (!pending && suggestedTick !== BigInt(market.current_tick || 0)) {
                // Schedule 24h notice
            ...
            } else if (MarketVolatilityService.isEmergencyWideningRequired(vol, Number(market.realized_volatility_24h || 0))) {
                // EMERGENCY WIDENING: Immediate apply
                const emergencyTick = suggestedTick > BigInt(market.current_tick || 0) ? suggestedTick : BigInt(market.current_tick || 0) * 2n;

                await supabase.from('markets').update({
                    current_tick: Number(emergencyTick),
                    pending_tick_change: null,
                    realized_volatility_24h: vol
                }).eq('id', market.id);
