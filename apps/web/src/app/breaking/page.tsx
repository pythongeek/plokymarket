/**
 * Breaking News Markets Page — ব্রেকিং নিউজ মার্কেট
 * Polymarket-style breaking news feed with Bengali content
 */
import { createPublicClient } from '@/lib/supabase/server';
import BreakingPageClient from './BreakingPageClient';
...
export default async function BreakingPage() {
  ...
  if (error) {
    console.error('[BreakingPage] Error:', error);
  }

  const breakingItems = (events || []).map((e) => {
  ...
  // Sidebar: top movers (highest prob change potential)
  const topMovers = breakingItems.slice(0, 5);
  ...
  return (
    <BreakingPageClient
      ...
      topMovers={topMovers}
      stats={{
        totalItems: toBengaliNum(breakingItems.length),
        totalVolume: formatVolumeBn(totalVol),
        activeNow: toBengaliNum(breakingItems.filter((_, i) => i < 5).length),
      }}
    />
  );
}
