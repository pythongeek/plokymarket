...
import {
  Search, Flame, TrendingUp, Clock, BarChart3,
  ChevronDown,
} from 'lucide-react';
...
const TOPIC_PILLS = [
  'সব', 'নির্বাচন', 'রাষ্ট্রিয় নেতা',
  'মার্কিন', 'যুক্তরাজ্য', 'ভারত',
  'বিশ্ব নেতা', 'মধ্যপ্রাচ্য নির্বাচন',
];
...
export default function PoliticsPageClient({ items }: Props) {
  ...
  const filtered = useMemo(() => {
    let result = [...items];
    if (activeTopic !== 'সব') {
      result = result.filter((m) => m.question.toLowerCase().includes(activeTopic.toLowerCase()));
    }
    if (search) {
  ...
  const visibleTopics = showMoreTopics ? TOPIC_PILLS : TOPIC_PILLS.slice(0, 6);
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Noto Sans Bengali', 'Hind Siliguri', sans-serif" }}>
      ...
      <div className="border-b border-gray-100">
        <div className="max-w-[1350px] mx-auto px-4 lg:px-6 py-6">
          <h1 className="text-2xl md:text-3xl font-black text-gray-900">রাজনীতি</h1>
          <p className="text-sm text-gray-500 mt-1">রাজনীতি সম্পর্কীত প্রেডিকশন মার্কেট ব্রাউজ করুন</p>
        </div>
      </div>
            ...
            {SORT_TABS.map((tab) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-[13px] font-semibold whitespace-nowrap transition-colors ${
                  activeSort === tab.key ? 'text-gray-900' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                {tab.label}
              </button>
