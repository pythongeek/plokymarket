...
export function MarketsClient({ initialEvents }: { initialEvents: Event[] }) {
    ...
    useEffect(() => {
        // We already hydrated initial events, now listen for deltas
        subscribeToEvents();
        // UseMarketStore manages unsub internally via unsubscribeAll, but we can try to clean up
        return () => {
            const u = useMarketStore.getState().unsubscribeEvents;
            if (u) u();
        };
    }, [subscribeToEvents]);

    // Derive the active list of markets from the zustand map
    const markets = useMemo(() => Array.from(storeEvents.values()), [storeEvents]);
    ...
    const processedMarkets = useMemo(() => {
        ...
        if (filters.status === 'active') {
            ...
            result = result.filter(m => m.status === 'active' && m.trading_closes_at && new Date(m.trading_closes_at) <= weekFromNow);
        ...
        if (!filters.search) {
            switch (filters.sort) {
                ...
                case 'ending':
                    ...
                    break;
                case 'price':
                    result.sort((a, b) => ((b as any).current_yes_price || (b as any).yes_price || 0.5) - ((a as any).current_yes_price || (a as any).yes_price || 0.5));
                    break;
            }
        }
        ...
        <div className="space-y-6">
            ...
            {processedMarkets.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {processedMarkets.map((event) => (
                        <MarketCard key={event.id} market={event as unknown as Market} />
                    ))}
                </div>
            ) : (
