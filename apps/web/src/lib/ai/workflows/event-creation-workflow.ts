...
async function executeCallbacksWithRotation<T>(
  primary: () => Promise<T>,
  secondary: () => Promise<T>,
  fallback: () => Promise<T>
): Promise<{ result: T; provider: AIProvider | "fallback" }> {
  try {
...
export interface WorkflowOutput {
  ...
  metadata: {
    ...
    providersUsed: (AIProvider | "fallback")[];
    timestamp: string;
  };
}
...
async function stage1SlugGeneration(input: WorkflowInput): Promise<WorkflowStage<SlugGenerationResult>> {
  ...
  try {
    const { result, provider } = await executeCallbacksWithRotation(
...
export function getWorkflowAnalytics(outputs: WorkflowOutput[]) {
  ...
  const providerDistribution = outputs.reduce((acc, o) => {
    ...
    return acc;
  }, {} as Record<AIProvider | "fallback", number>);
