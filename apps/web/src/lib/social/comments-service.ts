...
export class CommentsService {
  ...
  async postComment(
    ...
    const { data: comment, error } = await (supabase
      ...
      .insert({
        ...
        score: this.calculateInitialScore(reputation as unknown as UserReputation)
      } as any)
  ...
  async getMarketComments(
    ...
    if (currentUserId && rawComments?.length) {
      ...
      votes?.forEach((v: any) => userVotes.set(v.comment_id, v.vote_type));
    }

    // Get user reputations for comment authors
    const userIds = [...new Set(rawComments?.map((c: any) => c.user_id).filter(Boolean) || [])] as string[];
    ...
    userBadges?.forEach((ub: any) => {
      if (!badgesMap.has(ub.user_id as string)) badgesMap.set(ub.user_id as string, []);
      badgesMap.get(ub.user_id as string)!.push(ub);
    });
  ...
  async voteComment(
    ...
    const { data: updatedComment } = await supabase.rpc('update_comment_score' as any, {
      p_comment_id: commentId
    });

    return {
      success: true,
      newScore: ((updatedComment as any)?.[0]?.score || 0) as number
    };
  }
  ...
  private async notifyMentions(
    ...
    const notifications = mentionedUsers.map((u: any) => ({
      user_id: u.id,
      sender_id: senderId,
      type: 'mention',
