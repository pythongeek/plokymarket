...
export class FeedService {
  ...
  async getPersonalizedFeed(
    ...
    const userPrefs = (preferences as any) || this.getDefaultPreferences(userId);
    ...
    if (filterTypes?.length) {
      query = query.in('type', filterTypes as any);
    }
    ...
    const scoredActivities = ((rawActivities as any[]) || [])
      .map(a => algorithm.calculateScore(a as unknown as Activity))
  ...
  async getPreferences(userId: string): Promise<FeedPreferences> {
    ...
    return (data as any) || this.getDefaultPreferences(userId);
  }
  ...
  async updatePreferences(
    ...
    if (error) throw error;
    return data as unknown as FeedPreferences;
  }
  ...
  async shareActivity(activityId: string, targetPlatform: 'twitter' | 'facebook' | 'copy'): Promise<string> {
    ...
    const username = (activity as any).users?.username || 'someone';
    let text = `Check out this activity by @${username} on Plokymarket!`;
    const data = activity.data as any;

    if ((activity.type as string) === 'market_movement' && data?.marketQuestion) {
      text = `Market moving: ${data.marketQuestion} 🚀`;
    } else if ((activity.type as string) === 'trader_activity' && data?.action) {
      text = `@${username} just ${data.action} ${data.marketQuestion ? 'on ' + data.marketQuestion : ''}`;
    }
