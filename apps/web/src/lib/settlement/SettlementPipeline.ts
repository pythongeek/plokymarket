...
export class SettlementPipeline {
  ...
  private async validateOracleConfirmation(request: SettlementRequest): Promise<void> {
    ...
    if (market.winning_outcome !== request.winningOutcome) {
  ...
  private async finalizeOutcome(request: SettlementRequest): Promise<void> {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        winning_outcome_price: 1.00,
        losing_outcome_price: 0.00
      })
      .eq('id', request.marketId);
  ...
  private async activateRedemptions(request: SettlementRequest): Promise<void> {
    ...
    const claims: SettlementClaim[] = (positions || []).map(pos => ({
      id: `claim-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      userId: pos.user_id,
      marketId: request.marketId,
      outcome: pos.outcome,
      shares: pos.shares,
      payoutAmount: pos.shares * 1.00, // $1 per winning share
      status: 'pending',
      optInAutoSettle: pos.auto_settle_opt_in || false,
  ...
  private async processAutoSettlements(marketId: string): Promise<SettlementBatch | undefined> {
    ...
    const batch: SettlementBatch = {
      ...
      marketId,
      claims: autoSettleClaims,
      totalAmount: autoSettleClaims.reduce((sum, c) => sum + c.payout_amount, 0),
  ...
  private async checkSettlementConfirmation(marketId: string, batchId?: string): Promise<void> {
    ...
    if (market?.settlement_status !== 'confirmed') {
  ...
  private async executeBatchSettlement(batch: SettlementBatch): Promise<void> {
    ...
    for (const claim of batch.claims) {
      await supabase.rpc('credit_wallet', {
        ...
        p_amount: claim.payout_amount - (relayerFee / batch.claims.length)
      });
    }
  ...
  async claimRedemption(userId: string, marketId: string): Promise<{
    ...
    // Calculate fee
    const relayerFee = this.gasConfig.relayerEnabled
      ? claim.payout_amount * (this.gasConfig.relayerSurchargePercent / 100)
      : 0;
    
    const netAmount = claim.payout_amount - relayerFee;
  ...
  async getStats(): Promise<{
    ...
    const totalPayout = claims?.reduce((sum, c) => sum + (c.payout_amount || 0), 0) || 0;
    const settledClaims = claims?.filter(c => c.status === 'claimed' || c.status === 'auto_settled').length || 0;
