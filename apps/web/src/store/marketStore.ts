        ...
        (set, get) => ({
          ...
          subscribeToEvents: () => {
            const unsubscribe = eventsService.subscribeToEvents((payload) => {
              ...
              switch (eventType) {
                case 'INSERT':
                  get().addEvent(newEvent as unknown as UnifiedEvent);
                  break;
                case 'UPDATE':
                  get().updateEvent((newEvent as unknown as UnifiedEvent).id, newEvent as unknown as Partial<UnifiedEvent>);
                  break;
                case 'DELETE':
                  get().removeEvent((oldEvent as unknown as UnifiedEvent).id);
                  break;
              }
            });
          ...
          subscribeToFeatured: () => {
            const unsubscribe = eventsService.subscribeToFeaturedEvents((payload) => {
              ...
              if (eventType === 'INSERT' || eventType === 'UPDATE') {
                const event = newEvent as unknown as UnifiedEvent;
