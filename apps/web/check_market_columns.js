...
async function checkMarketColumns() {
    return new Promise((resolve) => {
        const req = https.request({
            hostname: 'sltcfmqefujecqfbmkvz.supabase.co',
            path: '/rest/v1/markets?select=id,question,status,yes_price,no_price,market_type,risk_score,stages_completed,current_stage,yes_price_change_24h&limit=1',
            method: 'GET',
            headers: {
                'apikey': ANON_KEY,
...
async function main() {
    console.log('=== Checking Market Columns ===\n');

    const result = await checkMarketColumns();
