-- ============================================================================
-- Migration 142: Production CLOB System (Better Than Polymarket)
-- ============================================================================
...
--   - MEV Protection
--   - Batch Orders (Bet Slip)
--   - Portfolio Margin Support
--   - Cross-Market Position Tracking
--   - Real-time Price Charts
--   - TradingView-Ready Data
--
-- Requirements:
--   - Must run AFTER: 007, 008, 021, 050, 102, 104, 117, 118
--   - Assumes CLOB tables exist: order_book, order_nodes, price_levels
--   - Assumes events/markets tables exist
--
-- ============================================================================
...
-- Add advanced order types to existing orders table
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS order_type VARCHAR(20) DEFAULT 'limit',
...
ALTER TABLE public.orders ADD CONSTRAINT orders_order_type_check 
  CHECK (order_type IN ('limit', 'market', 'stop_loss', 'take_profit', 'trailing_stop', 'iceberg'));
...
CREATE INDEX IF NOT EXISTS idx_orders_stop_price 
  ...
  WHERE order_type IN ('stop_loss', 'take_profit') AND status = 'trigger_pending';
...
BEGIN
  ...
  IF NEW.status NOT IN ('open', 'partial') THEN
    RETURN NEW;
  END IF;
