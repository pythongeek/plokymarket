-- ============================================================================
-- Migration 142b: Production CLOB System (Better Than Polymarket)
-- ============================================================================
-- This migration creates a production-ready Central Limit Order Book (CLOB)
-- 
-- PREREQUISITE: Must run 142a_extend_order_type_enum.sql FIRST
--
-- ============================================================================
...
-- Index for stop-loss/take-profit orders (safe to use now that enum is extended)
CREATE INDEX IF NOT EXISTS idx_orders_stop_price 
  ON public.orders(stop_price, trigger_condition, status) 
...
DROP POLICY IF EXISTS "Public can view resolution systems" ON public.resolution_systems;
CREATE POLICY "Public can view resolution systems"
  ON public.resolution_systems FOR SELECT USING (true);

DROP POLICY IF EXISTS "Admins can manage resolution systems" ON public.resolution_systems;
...
DROP POLICY IF EXISTS "Public can view active categories" ON public.custom_categories;
CREATE POLICY "Public can view active categories"
  ON public.custom_categories FOR SELECT USING (is_active = TRUE);

DROP POLICY IF EXISTS "Admins can manage categories" ON public.custom_categories;
...
-- Ensure volume_24h column exists (for existing tables)
ALTER TABLE public.price_history ADD COLUMN IF NOT EXISTS volume_24h NUMERIC DEFAULT 0;
...
-- Using subqueries for open/close to avoid window function + GROUP BY issues
...
SELECT 
  ...
  h.outcome,
  h.hour,
  -- Open price: first price in the hour
  (SELECT price FROM public.price_history ph 
   WHERE ph.market_id = h.market_id 
     AND ph.outcome = h.outcome 
     AND date_trunc('hour', ph.recorded_at) = h.hour 
   ORDER BY ph.recorded_at ASC LIMIT 1) AS open_price,
  h.high_price,
  h.low_price,
  -- Close price: last price in the hour
  (SELECT price FROM public.price_history ph 
   WHERE ph.market_id = h.market_id 
     AND ph.outcome = h.outcome 
     AND date_trunc('hour', ph.recorded_at) = h.hour 
   ORDER BY ph.recorded_at DESC LIMIT 1) AS close_price,
  h.volume
FROM hourly h
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 142b (Production CLOB System) applied successfully';
